...
var Options = {
  ...
  codec: undefined,       // content-type (mime) eg: application/ldjson (json), application/amp-message (npm install amp-message).
  charset: 'utf8',
  slowHandshake: false,   // true: if you wish to manage own auth at app level.
  timedout: 15000         // how to to wait for connection.
};
...
function createServer(options, fn) {
  ...
  self.server = ss.createServer(options, function(socket) {
    ...
    if ( options.noDelay ) {
      socket.setNoDelay(true);   // turn nagle batching alguruthum off.
    }
...
function connect(options, fn) {
  ...
  if ( options.noDelay ) {
    self.socket.setNoDelay(true);   // turn nagle batching alguruthum off.
  }
...
function clientParser(self, buffer, chunk, socket, _fn) {
  ...
  // TODO: add timeout if yet to send header in time.
